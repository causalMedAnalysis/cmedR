    ...
    lapply(
      ...
      function(rst_df){
        rst_df <-
          ...
          mutate(
            estimand = case_when(
              estimand == "direct" ~ "PSE(D -> Y)",
              estimand == "total" ~ "ATE(1,0)",
              str_detect(estimand, "via M\\d+") ~ 
                if_else(
                  str_detect(estimand, paste0("M", length(M))),
                  str_replace(estimand, "via (M\\d+)", "PSE(D -> \\1 -> Y)"),
                  str_replace(estimand, "via (M\\d+)", "PSE(D -> \\1 ~> Y)")
                ),
              TRUE ~ estimand
            ),
            est = round(estimate, round_decimal),
            intv = paste0("[", round(lower, round_decimal), ", ", round(upper, round_decimal), "]"),
            out = paste(est,intv)
          ) %>%
          mutate(
            order_key = case_when(
              str_detect(estimand, "ATE") ~ 1,         
              estimand == "PSE(D -> Y)" ~ 2,           
              TRUE ~ 3                                
            )
          ) %>%
